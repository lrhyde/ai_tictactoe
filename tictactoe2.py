# -*- coding: utf-8 -*-
"""
Created on Tue Nov 23 13:40:19 2021
...
def choose_move(board):
    best_result = max_step(board, computerToken)
    ind = 0
    i = 0
    ilist = indexList(board, 2)
    for next_board in possible_next(board, computerToken):
        k = -1*max_step(next_board, playerToken)
        k2 = ""
        if(k==0): k2 = "tie"
        if(k==1): k2 = "win"
        if(k==-1): k2 = "loss"
...
def max_step(board, player):
    if(not gameOver(board)==None):
        if(gameOver(board)=="full"): return 0
        elif(gameOver(board)==player): return 1
        else: return -1
    otherPlayer = 'O'
    if(player=='X'):
        otherPlayer = 'O'
    ...
    results = []
    for next_board in possible_next(board, player):
        results.append(-1*max_step(next_board, otherPlayer))
    return max(results)
    
def possible_next(board, player):
    boardList = []
